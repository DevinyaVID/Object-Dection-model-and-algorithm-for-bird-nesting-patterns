...
def process_video():
    ...
    # Check for reference incubation radius
    reference_incubation_radius = calculate_reference_incubation_radius(LABELS_FOLDER, class_names)
    if reference_incubation_radius is None:
        # No eggs detected in any initial frames, invalid video case
        return jsonify({"message": "Invalid video: No detections found"}), 400

    # Process frames and classify behaviors
    ...
    if not results:
        return jsonify({"message": "Invalid video: No detections found in any frames"}), 400
